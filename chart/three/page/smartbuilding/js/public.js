...
renderer.setClearColor('rgb(0,0,0)',1.0);///背景
...
function createBasicCuboid({size,position,rotate,border,materials,remove}){ ///a => x 长,b => z 宽,h => y高
    ...
    if(materials.length == 6)
       ...
       material = materials.map(item => new THREE.MeshBasicMaterial(item));
    ...
    if(border){
        ...
        let edgesMtl =  new THREE.LineBasicMaterial({color: 0x72e4fd});
...
function createPhongCuboid({size,position,rotate,border,materials,remove}){ ///a => x 长,b => z 宽,h => y高
